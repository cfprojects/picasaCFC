<!---
	...
	Description		: App allowing users to read in RSS from Picasa.

	------------Method Listing------------------
	Name: Init
			Returntype:	picasaCFC
			Accesslevel:	public
			Arguments:	username (string, required);
								gallerysize (numeric)
								albumsize (numeric)
								largesize (numeric)
								cropgallerythumb (string)
								cropphotothumb (string)
	...
	Name: getAlbum
			Returntype:	array
			Accesslevel:	public
			Arguments:	rsslink (string, required)
	...
	Name: forceValue
			Returntype:	string
			Accesslevel:	private
			Arguments:	input (numeric, required)
--->
